...
#     test_temp = np.array(X**2);
#     test_temp = np.sum(test_temp, axis = 1); #(500,)
    
#     train_temp = np.array(self.X_train**2)
#     train_temp = np.sum(train_temp, axis = 1) #(5000,)
    
#     train_temp = train_temp.reshape(num_train, 1) + test_temp
#     #test_temp = test_temp.reshape(num_test, 1) + train_temp
#     dists = np.sqrt(train_temp.T - 2*np.dot(X, self.X_train.T))
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in np.arange(num_test):
	  ...
	  #   as y_pred[i].  Break ties by choosing the smaller label.
        ...
        closest_y = self.y_train[sortedIndex[:k]]
