...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        #   The dimensions of W1 should be (input_dim, hidden_dims) and the
    ...
    """
        ...
        loss = loss_soft + 0.5 * self.reg * (np.sum(self.params['W1'] **2) + np.sum(self.params['W2'] ** 2))
...
class FullyConnectedNet(object):
  ...
  """
    ...
    """
        ...
        total_dims = [input_dim].append(hidden_dims)
        total_dims.append([num_classes])
        for i in range(self.num_layers):
            self.params['b' + str(i + 1)] = np.zeros((total_dims[i + 1], 1))
    ...
    """
        ...
        input_x[0] = [X]
        for i in range(self.num_layers):
            output_y[i + 1] = affine_forward(input_x[i][0], self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
            h[i + 1] = relu_forward(param[i + 1][0])
        scores = output_y[self.num_layers][0]
        ...
        for weight in weights:
            loss = soft_loss 0.5 * self.reg * sum(np.sum(weight**2))
        dys[self.num_layers] = dx

        for i in range(self.num_layers)[::-1]:
            dh, dw, db = affine_backward(dys[i + 1], param[i + 1][1])
            dhs[i] = dh      
            dws[i + 1] = dw
            dbs[i + 1] = db
            if i != 0:
                dys[i] = relu_backward(dhs[i], h[i][1])
