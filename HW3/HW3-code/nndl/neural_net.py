...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        scores_log = np.log(np.sum(np.exp(norm_scores), axis=1))
        softmax_loss = scores_log - norm_scores[np.arange(N), y]
        softmax_loss = np.mean(softmax_loss)
        l2 = np.sum(W1 * W1) + np.sum(W2 * W2)
        loss = softmax_loss + 0.5 * reg * l2
        # ================================================================ #
        # END YOUR CODE HERE
        # ================================================================ #
        ...
        dLdS = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)
        dLdS[np.arange(N), y] -= 1
        dLdS = dLdS / N

        grads["b2"] = np.sum(dLdS, axis=0)
        grads["W2"] = dLdS.T.dot(reLu) + reg * W2

        dLdR = W2.T.dot(dLdS.T)
        dLdR = (h1 > 0).T * dLdR

        grads["b1"] = np.sum(dLdR, axis=1).T
        grads["W1"] = dLdR.dot(X) + reg * W1
